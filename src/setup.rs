...
use crate::{runtime::{RuntimeDescriptor, RuntimeDownloadFormat, write_runtime_id}, gui::{run_progress_dialog, ProgressDialog}};
...
pub enum SetupError {
    #[error("Unable to connect to the runtime download server '{0}': {1}")]
    DownloadServerUnreachable(String, ErrorBox),

    #[error("Failed to initialize the async runtime: {0}")]
    AsyncRuntimeError(ErrorBox),

    #[error("Failed to open the progress dialog: {0}")]
    ProgressDialogError(ErrorBox),
...
pub fn setup_runtime(target_id: &str, runtime_descr: &RuntimeDescriptor, runtime_dir: &Path) -> Result<(), SetupError> {
    ...
    if let Some(download_host) = download_url.host() {
        if let Some(port) = download_url.port_or_known_default() {
            ...
            if let Err(e) = TcpStream::connect((download_host.as_ref(), port)) {
                ...
                return Err(SetupError::DownloadServerUnreachable(download_host, Box::new(e)));
            }
        }
    }
    ...
    let Some(diag_res) = run_progress_dialog::<Result<(), AsyncSetupError>>(&diag_descr, move |dialog| {
        ...
        if !runtime_descr.download_sha512.eq(runtime_hash) {
            ...
            println!("Unexpected download hash: {} != {}", expected_hash, actual_hash);
            return Err(AsyncSetupError::DownloadHashMismatch(expected_hash, actual_hash));
        }
        println!("Downloaded runtime hash matches expected hash");
        ...
        dialog.set_progress("Finalizing", 1_f64);
        ...
        println!("Successfully set up runtime version {ver} for target '{target_id}' in '{dir}'", ver=runtime_descr.version, dir=runtime_dir.display());
        Ok(())
    }).map_err(SetupError::ProgressDialogError)? else {
...
fn download_runtime(dialog: &ProgressDialog, async_runtime: &Runtime, target_id: &str, runtime_descr: &RuntimeDescriptor) -> Result<Bytes, CrossThreadErrorBox> {
    async_runtime.block_on(async move {
        ...
        println!("Downloading runtime '{target_id}' from '{}' ({})...", runtime_descr.download_url, ByteSize::b(content_len));
        ...
        while let Some(chunk) = stream.next().await {
            ...
            if dialog.is_cancelled() { return Ok(Bytes::default()); }

            //Append the chunk to the buffer
            data.put_slice(&chunk);

            //Update the progress bar
            dialog.set_progress(format!("Downloading runtime '{target_id}': {}/{}", ByteSize::b(data.len() as u64), ByteSize::b(content_len)), (data.len() as f64) / (content_len as f64));
        }
...
fn decompress_targz_runtime(dialog: &ProgressDialog, runtime_dir: &Path, data: &Bytes) -> Result<(), CrossThreadErrorBox> {
    ...
    println!("Unpacking TAR ({num_entries} entries)...");
    ...
    for entry in archive.entries()? {
        ...
        //Update the progress bar
        dialog.set_progress(format!("Unpacking archive: {num_unpacked}/{num_entries}"), (num_unpacked as f64) / (num_entries as f64));
    }

    Ok(())
}

fn decompress_zip_runtime(dialog: &ProgressDialog, runtime_dir: &Path, data: &Bytes) -> Result<(), CrossThreadErrorBox> {
    fs::create_dir_all(runtime_dir)?;
